...
> 文档版本：V1.3
> 确定日期：2026年9月4日
> 当前实现：导入校验、批次追溯、独立题目入库、TMUA Paper 分类和题目替换发布均已启用。

## 1. 设计目标

试题库中的每道题都是独立业务对象，可以被单独导入、编辑、审核、发布、归档和删除，也可以按照考试类型、难度和考纲分类进入专项练习或练习本选题。
...
- 新导入题目统一进入 draft 状态，上传文件不能直接发布题目；替换题审核后必须按上传包整包上线。
...
| revision | 否 | object | 修正既有题目时声明直接前一版本、目标版本和修正原因 |
...
`revision` 只用于修正已经入库并发布的题目。普通新题不得填写；它与 `origin` 可以同时存在，两者含义不同：`origin` 描述内容来源，`revision` 描述当前题替换哪个既有题目。

标准结构：

    {
      "originCode": "ESAT-MOCK-Q012",
      "version": "V2",
      "reason": "corrected_content"
    }

| 字段 | 是否必需 | 标准要求 |
| --- | --- | --- |
| originCode | 是 | 当前试题库中直接前一版本的精确 `code`；该题必须存在、已发布，并与新题考试类型和模块一致 |
| version | 是 | `V2`、`V3` 等大于等于 2 的连续版本号，必须恰好是直接前一版本加 1 |
| reason | 是 | 小写 snake_case 修正原因，例如 `corrected_content`、`corrected_answer`、`corrected_diagram` |

新题 `code` 必须使用基础 code 加目标版本号。若前一版本是 `ESAT-MOCK-Q012`，V2 为 `ESAT-MOCK-Q012-V2`；若前一版本是 `ESAT-MOCK-Q012-V2`，V3 仍为 `ESAT-MOCK-Q012-V3`，不会叠加为 `-V2-V3`。

替换发布规则：

- 同一文件中每个 `originCode` 只能出现一次，且不能指向本文件中新建的题目。
- 导入只创建新题草稿，不修改原题，也不立即影响学生作答。
- 带 `revision` 的题不能使用单题发布，必须从所属上传包整包上线。
- 整包上线在同一事务内发布新题、归档直接前一版本，并更新没有开放且没有答卷的模考草稿。
- 已发布、已有单项发布或已有答卷的模考不会原地改题；系统以相同 `examType + sequenceNo` 创建内部新版本，归档旧版并保留旧题、旧答卷、报告和错题历史。
- 学生端仍显示稳定的 `No.001` 等逻辑编号。未做用户进入当前新版；做过旧版的用户仍显示已完成并查看旧版历史，选择“重新做”时创建当前新版答卷。
...
7. 校验题干、选项、答案和图片引用完整。
8. 校验中文解析不为空且无乱码。
9. `revision` 存在时，校验直接前一版本已发布、考试类型和模块一致，并建立不可变替换关系。
10. 所有题目校验通过后，在单个数据库事务中创建导入批次、题目和知识点关联。
11. 任意一步失败时回滚整个导入，不保留部分成功数据。

重复 code 的默认处理是拒绝整批导入，不执行自动更新或覆盖。

已有答题记录的题目不得覆盖题干、选项、答案、图片、考试类型或考纲分类。如需修改影响判分和展示的内容，应创建新的 code。

## 20. JSON 与 JSON-in-Markdown
...
- revision 如存在，originCode、version、reason 完整，新 code 与版本规则一致。
- 文件不包含试卷结构、系统状态或调试字段。
- 文件是严格 UTF-8 JSON。
...
- 替换题发布必须原子完成题目换版与模考引用换版，不能改写旧答卷引用的 Question 或 Paper。
